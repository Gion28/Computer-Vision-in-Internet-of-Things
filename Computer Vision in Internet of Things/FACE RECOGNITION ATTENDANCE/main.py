...
#Function untuk mengecek directory dan jika tidak ada directory, maka akan dibuat foldernya
def assure_path_exists(path):
    dir = os.path.dirname(path)
    if not os.path.exists(dir):
        os.makedirs(dir)

#Functions untuk mengambil waktu
def tick():
    time_string = time.strftime('%H:%M:%S')
    clock.config(text=time_string)
    clock.after(200,tick)

#Function untuk menentukan pilihan di bagian navigation bar
def contact():
    mess._show(title='Credit', message="Comvis Kelompok 8")
...
#Function untuk menyimpan password dan mengganti password
def save_pass():
    assure_path_exists("TrainingImageLabel/")
    exists1 = os.path.isfile("TrainingImageLabel\psd.txt")
    if exists1:
        #Membuka file txt dan membaca file tersebut
...
#Function untuk menampilkan pengubahan password di GUI
def change_pass():
    global master
    master = tk.Tk()
...
#Function untuk menyimpan Nama, NIM, dan Gambar Muka User
def TakeImages():
    ...
    if ((name.isalpha()) or (' ' in name)):
        ...
        sampleNum = 0
        while (True):
            #Akan dilakukan pembacaan wajah
            ret, img = cam.read()
            ...
            for (x, y, w, h) in faces:
                ...
                cv2.imwrite("TrainingImage\ " + name + "." + str(serial) + "." + nim + '.' + str(sampleNum) + ".jpg",
                            gray[y:y + h, x:x + w])
                # display the frame
...
#Function untuk menampilkan registerasi, profile sudah disimpan, dan jumlah yang meregisterasi
def TrainImages():
    check_haarcascadefile()
    assure_path_exists("TrainingImageLabel/")
...
def getImagesAndLabels(path):
    ...
    #Membuat list untuk face
    faces = []
    #Membuat list untuk nim
    Ids = []
    for imagePath in imagePaths:
        pilImage = Image.open(imagePath).convert('L')
        #Mengubah PIL image ke numpy array
        imageNp = np.array(pilImage, 'uint8')
        #Mengambil id dari gambar
        ID = int(os.path.split(imagePath)[-1].split(".")[1])
        #Menambahkan face ke dalam training image sample
        faces.append(imageNp)
        Ids.append(ID)
    return faces, Ids

#Function untuk menyocokan gambar, jika sesuai maka akan dimasukan ke dalam file attendance dan student detail
def TrackImages():
    check_haarcascadefile()
    #Memastikan path attendace
    assure_path_exists("Attendance/")
    #Memastikan path detail student
    assure_path_exists("StudentDetails/")
...
filemenu.add_command(label='Credit', command = contact)
filemenu.add_command(label='Exit',command = window.destroy)
menubar.add_cascade(label='Help',font=('times', 29, ' bold '),menu=filemenu)

#Untuk treeview attendance table
